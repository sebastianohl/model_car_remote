    ...
    while(1) {
        ...
        if(xQueueReceive(car_config.gpio_evt_queue, &value, 500/portTICK_RATE_MS)) {
            switch (value.channel_idx)
            {
                ...
                case 1: {
                    modelcar_update_drivemode(&car_config.drive_mode[1], value.pulse_width, modelcar_httpd_get_servo2_offset());
                    uint32_t modified_dc = modelcar_update_output_by_us(
                        &car_config.output_channel[1], 
                        value.pulse_width, 
                        (car_config.drive_mode[1] >= BREAK)?1:modelcar_httpd_get_servo2_factor(),
